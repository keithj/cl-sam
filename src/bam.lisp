...
(define-alignment-tag :bc :string
  "Barcode sequence, with any quality scores stored in the QT tag.")
...
(define-alignment-tag :co :string
  "Free-text comments.")
...
(define-alignment-tag :ct :string
...
(define-alignment-tag :pt :string
...
(define-alignment-tag :qt :string
  "Phred quality of the barcode sequence in the BC (or RT) tag. Same encoding as QUAL.")
...
(define-alignment-tag :rg :string
  (txt "Read group. Value matches the header RG-ID tag if @RG is present"
       "in the header."))
(define-alignment-tag :rt :string
  "Deprecated alternative to BC tag originally used at Sanger.")
(define-alignment-tag :sa :string
  (txt "Other canonical alignments in a chimeric alignment, in the format of:"
       "(rname,pos,strand,CIGAR,mapQ,NM;)+. Each element in the semi-colon"
       "delimited list represents a part of the chimeric alignment."
       "Conventionally, at a supplementary line, the first element points to"
       "the primary line."))
...
(define-alignment-tag :xc :int32 "BWA: undocumented")
...
(define-alignment-tag :xn :int32 "BWA: number of ambiguous bases")
